contributionsADD <- function(reg_name, cat_table = "var categorization table.xlsx", lag_rollback = FALSE,
                             ...
                             plm_is_additive = TRUE, displayMeanKPI = FALSE, TIV = FALSE) {
  ...
  # reg_name = "murdiv11"
  # cat_table = "var categorization table mur.xlsx"
  ...
  # custom_period_table="customPeriod.csv"
  # mQED=FALSE
  # using_plm=FALSE
  # plm_coefficents=NULL
  # plm_is_additive=TRUE
  # displayMeanKPI=FALSE
  ...
  if (!using_plm && !mQED) {
    ...
    if (stringr::str_detect(names(reg_gr)[3], "LOG\\(")) {
      model.type <- "LOG"
    } else {
      model.type <- "LIN"
    }
  ...
  if (aggregation_period == "CustomPeriod") {
    ...
    if (base::class(custom_period_table) == "character") {
      cPdf <- suppressMessages(suppressWarnings(readr::read_csv(custom_period_table))) %>%
        ...
        dplyr::transmute(
          Date = as.Date(lubridate::parse_date_time(.[[1]], orders = c("dmy", "mdy"))),
  ...
  if (TIV) {
    reg_gr2 <- base::cbind(base::t(base::as.data.frame(stringr::str_split(reg_gr2[[1]], " - ", 2))), reg_gr2[, 2:base::ncol(reg_gr2)])
    base::row.names(reg_gr2) <- NULL
    base::names(reg_gr2) <- c("Group", "Date", base::names(reg_gr2[3:base::ncol(reg_gr2)]))


    reg_gr_TIV <- reg_gr2
    # reg_gr[,-4]
    reg_gr_TIV$Date <- base::as.Date(lubridate::parse_date_time(reg_gr_TIV$Date, orders = c("dmy", "mdy", "ym")))

    if (lubridate::day(reg_gr_TIV$Date + 1) %>% base::mean() == 1) reg_gr_TIV$Date <- reg_gr_TIV$Date + 1

    reg_gr_TIV$TIV <- reg_gr_TIV[, 3] / reg_gr_TIV[, 4]
    pg <- reg_gr_TIV
    reg_gr_TIV <- reg_gr_TIV[, -c(3:(length(reg_gr_TIV) - 1))]
  }
  ...
  for (q in 1:base::length(reg_gr_list)) {
    ...
    if (!using_plm && !mQED) {
      ...
      reg_gr$Date <- base::as.Date(reg_gr$Date)
    }
    # print(names(reg_gr))
    ...
    for (b in 2:base::ncol(reg_gr)) {
      ...
      for (i in (1:base::length(incomplete))[incomplete]) {
        ...
        missingValues[[counterMissingValues]] <- base::as.data.frame(base::t(base::c(
          base::as.character(groups)[q],
    ...
    if (model.type == "LIN") {
      ...
      if (base::length(ref_base_correction) != 1) {
    ...
    if (q == 1) { ## seperate calc for first loop
      ...
      if (using_plm) {
        ...
        eq <- plm_coefficents %>%
          dplyr::transmute(
            Variable = stringr::str_to_upper(base::rownames(plm_coefficents)),
      ...
      categorization %>%
        ...
        dplyr::mutate(
          variable = stringr::str_trim(stringi::stri_trans_toupper(Variable, locale = "")),
    ...
    if (model.type == "LIN") {
      ...
      if (base::length(coef_base_correction) != 1) {
        coef_base_correction <- coef_base_correction[-1]
      }
    ...
    litenstor <- base::as.matrix(purrr::map_df(contribution, function(x) {
      x > sum(reg_gr[, 2]) | x < (-sum(reg_gr[, 2]))
    }))
    litenstor[, 1] <- FALSE
    ...
    if (q == 1) { ## seperate calc for first loop
      ...
      if (base::class(reg_name) == "character") {
        var_cat_tbl$Model <- reg_name
      } else {
    ...
    contribution <- contribution %>%
      ...
      dplyr::mutate(
        HY = base::ifelse(lubridate::quarter(Date) < 3, "1", "2"),
        QY = lubridate::quarter(Date)
      ) %>%
      dplyr::mutate(
        HalfYear = base::paste0(lubridate::year(Date), " H", HY),
    ...
    if (base::length(groups) == 1) {
      ...
      summary10 <- summary10 %>%
        group_by(Group, variable, FiscalYear) %>%
      ...
      summary11[, -(1:2)] <- summary11[, -(1:2)] / rowSums(summary11[, -(1:2)])
      summary11 <- gather(summary11, "variable", "value", 3:ncol(summary11))
      summary11$Group <- "Brand"
      summary11 <- summary11 %>%
        group_by(Group, variable, agg_period_) %>%
        summarise(cont = sum(value)) %>%
      ...
      # Multiply by TIV
      filter(reg_gr_TIV, Group == groups[q]) -> reg_group_tiv
      summary10 %>%
        left_join(reg_group_tiv, by = c("Group", "Date")) %>%
        mutate(value = value * TIV) -> summary10

      # Return KPI
      summary10$value[summary10$variable == names(reg_gr)[2]] / summary10$TIV[summary10$variable == names(reg_gr)[2]] -> summary10$value[summary10$variable == names(reg_gr)[2]]

      summary10 <- summary10 %>%
        group_by(Group, variable, Date) %>%
        summarise(cont = sum(value)) %>%
        reshape2::dcast(Group + variable ~ Date, fun.aggregate = sum, value.var = "cont")
    ...
    # reg_gr_TIV %>%
    #   filter(Group == groups[p]) %>%
    #   mutate(FiscalYear = FY(Date,FY_Begins)) %>%
    #   group_by(Group, FiscalYear) %>%
    #   summarise(TIV = sum(TIV)) -> reg_summary
    # reg_summary$TIV*summary[, 3:ncol(summary)] -> sum_temp
    # cbind(summary[,1:2], sum_temp) -> summary
    summary2 <- summary
    # print(summary)
    summary2[, -(1:2)] <- summary2[, -(1:2)] / rowSums(summary2[, -(1:2)])
    ...
    if (TIV) {
      contribution <- contribution
      contribution$Group <- groups[q]
      contribution <- contribution %>% left_join(reg_gr_TIV, by = c(Group = "Group", Date = "Date"))

      for (i in 4:(length(contribution) - 2)) {
        contribution[, i] <- contribution[, i] * contribution[, length(contribution)]
      }
      contribution$Group <- NULL
      contribution$TIV <- NULL
    }


    if (remove_base) contribution$BASE <- 0
    # recalculate base
    contribution[, 3] - rowSums(contribution[, 5:ncol(contribution)]) -> contribution[, 4]
    contribution2 <- contribution

    contribution2[, -(1:3)] <- contribution2[, -(1:3)] / rowSums(contribution2[, -(1:3)])
    ...
    if (q == 1) {
      contribution_return <- dplyr::bind_cols(base::as.data.frame(base::matrix(data = groups[q], nrow = nrow(contribution), ncol = 1, dimnames = base::list(
        base::c(),
        base::c("Group")
      )), stringsAsFactors = FALSE), contribution)

      contribution2_return <- dplyr::bind_cols(
        base::as.data.frame(base::matrix(data = groups[q], nrow = nrow(contribution2), ncol = 1, dimnames = base::list(base::c(), base::c("Group"))), stringsAsFactors = FALSE),
        contribution2
      )
      summary_return <- dplyr::bind_cols(base::as.data.frame(base::matrix(
        data = groups[q],
        nrow = nrow(summary), ncol = 1, dimnames = base::list(base::c(), base::c("Group"))
      ), stringsAsFactors = FALSE), summary)
      summary2_return <- dplyr::bind_cols(base::as.data.frame(base::matrix(
        data = groups[q],
        nrow = nrow(summary2), ncol = 1, dimnames = base::list(base::c(), base::c("Group"))
      ), stringsAsFactors = FALSE), summary2)
      if (print_summary == TRUE) base::print(var_cat_tbl)
    } else {
      contribution <- dplyr::bind_cols(base::as.data.frame(base::matrix(
        data = groups[q],
        nrow = nrow(contribution), ncol = 1, dimnames = base::list(base::c(), base::c("Group"))
      ), stringsAsFactors = FALSE), contribution)
      contribution2 <- dplyr::bind_cols(base::as.data.frame(base::matrix(
        data = groups[q],
        nrow = nrow(contribution2), ncol = 1, dimnames = base::list(base::c(), base::c("Group"))
      ), stringsAsFactors = FALSE), contribution2)
      summary <- dplyr::bind_cols(base::as.data.frame(base::matrix(
        data = groups[q],
        nrow = nrow(summary), ncol = 1, dimnames = base::list(base::c(), base::c("Group"))
      ), stringsAsFactors = FALSE), summary)
      summary2 <- dplyr::bind_cols(base::as.data.frame(base::matrix(
        data = groups[q],
        nrow = nrow(summary2), ncol = 1, dimnames = base::list(base::c(), base::c("Group"))
      ), stringsAsFactors = FALSE), summary2)
  ...
  if (base::length(missingValuesList) > 0) {
    if (imputeMissingValues) {
      base::message(
        "The following datapoints were imputed due to missing values:\n",
        printAndCapture(dplyr::bind_rows(missingValuesList))
      )
    }
    if (!imputeMissingValues) {
      base::message(
        "The following periods were excluded due to missing values:\n",
        printAndCapture(dplyr::bind_rows(missingValuesList)),
