contributionsADD <- function(reg_name, cat_table="var categorization table.xlsx",lag_rollback=FALSE,
  ...
  for(q in 1:base::length(reg_gr_list)){
    ...
    if(base::length(groups)==1){
      ...
      summary10 %>% left_join(reg_group_tiv, by = c("Group", "Date")) %>% mutate(value = value * TIV) -> summary10
      ...
      summary10 <- summary10 %>%  group_by(Group, variable, Date) %>%
        summarise(cont = sum(value)) %>%
      ...
      summary11 <- summary11 %>% group_by(Group, variable, Date) %>%
        summarise(cont = sum(value)) %>%
        reshape2::dcast(Group + variable ~ Date, fun.aggregate = sum, value.var = "cont")
    ...
    summary <-contribution %>% select(-Date) %>% group_by(FiscalYear) %>% summarise_all(sum)
    summary2 <- summary
    summary2[,-(1:2)] <- summary2[,-(1:2)]/rowSums(summary2[,-(1:2)])
  ...
  return(base::list(contributionsTbl=contribution_return,
                    ...
                    tailsDF=tailsPrep,
                    pg=pg))  #Return contribution DF from function
}
