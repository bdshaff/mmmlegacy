unwind <-function(model1,model2,use_model2_panel="ALL",print_summary=TRUE,expandPeriodToLongest=FALSE,demo=FALSE){
  ...
  for(q in 1:base::length(main_m_groups)){
    ...
    for(m in base::names(sub_m)[-(1:3)]) {if(m != "BASE") new_m1[,m]<-main_var_for_sub*sub_m[,m]} #loop for distributing contributions for kpi in sub model in main model
  ...
  if(base::length(main_m_groups)!=1){
    ...
    if(print_summary==TRUE) {base::names(tmp)[2] <- agg_period_
    base::print(tmp)}
  }
  ...
  return(list(contributionsTbl=contribution_return,
              ...
              varCont=var_tblPG,
              seankeyReadyTbl=sank))#Return contribution DF from function
}
