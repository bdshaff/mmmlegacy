ABCsReachGen = function(Contrib, 
  ...
  Contrib1 = Contrib1 %>% filter(!is.na(Spend)) %>% filter(Contributions > 0)
  ...
  Data = Data %>% mutate(Week = as.character(Week)) %>% as_tibble()
  ...
  AllReach1 = 
    ...
    mutate(TableName = str_split_fixed(Category, " ", n = 2)[,1],
  ...
  abcs = abcs %>% bind_rows %>% select(estimate)
